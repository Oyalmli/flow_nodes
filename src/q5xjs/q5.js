...
export default function Q5(scope, parent){
  "use strict";
  return new graphics(scope, parent);
  function graphics(scope, parent){let $ = (scope == "global" ? window : this);
    ...
    if (scope != "offscreen"){
      if (parent) {
        ...
        window.addEventListener("load", function _load(){
          parent.appendChild($.canvas);
          window.removeEventListener("load", _load);
        })
      }
    }
    ...
    $._pixelDensity = window.devicePixelRatio;
    ...
    $._setCanvas = function(width, height) {
      $.width = width;
      $.height = height;
      $.canvas.width = width;
      $.canvas.height = height;
    ...
    function _draw(){
      ...
      $._actualFrameRate = 1000. / ($._now - $._last)
      $._last = $._now
